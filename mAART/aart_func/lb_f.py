...
def spacedmarks(x, y, Nmarks):
    ...
    arclength = cumulative_trapezoid(sqrt(dydx**2 + dxdx**2), initial=0) # ***Changed cumtrapz to cumulative_trapezoid***
...
def hulls(alpha, beta, spin_case, thetao, smin=0.5, smax=100,limi0=0.99,lime0=1.01,limi1=0.999,lime1=1.001,limi2=0.9999,lime2=1.001):
...
def lb(spin_case,thetao):

    #"Computing the grids for each lensing bands"
    critc=CritCurve(spin_case,thetao)

    alpha_critc, beta_critc = spacedmarks(critc[0], critc[1], npointsS)

    hull_0i, hull_0e, hull_1i, hull_1e, hull_2i, hull_2e = hulls(alpha_critc,beta_critc,spin_case,thetao)
    ...
    filename=path+"LensingBands_a_%f_i_%f.h5"%(np.round(spin_case,6),np.round(thetao,6))
