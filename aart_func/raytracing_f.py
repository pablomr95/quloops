...
def rt(supergrid0,mask0,supergrid1,mask1,supergrid2,mask2,spin_case,thetao):
    """
    ...
    rs0,sign0,t0,phi0 = calculate_observables(supergrid0,mask0,np.round(thetao,6),np.round(spin_case,6),0,distance=D_obs)
    
    filename=path+"Rays_a_%f_i_%f.h5"%(spin_case,thetao)
    h5f = h5py.File(filename, 'w')
