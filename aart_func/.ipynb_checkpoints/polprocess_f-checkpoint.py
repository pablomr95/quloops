...
def generate_trajectory(a, beta_r, beta_phi, subkep, r_init, phi_init):
    """
    ...
    target_phi = phi_init + 2 * np.pi*0.9999 #phi_init + N_turns * 2 * np.pi
    ...
    t_span = (0, 2000) # Arbitrary large time, will stop at event
...
def process_polarization(thetao, spin_case, Br, Bth, Bphi, betar, betaphi, sub_kep, gfactor, traj,rs0,phi0,EVPA_x,EVPA_y,gTotal):
    """
    ...
    U_vals = -(-2 * ealpha * ebeta).real #Overall minus sign rotates the image (as seen from below or for an observer with i_case > pi/2)

    # Prepare Grid Points
    r_coords = np.array(rs0)
    phi_coords = np.array(phi0 + np.pi/2) % (2 * np.pi)

    # Only include grid points that are not NaN
    mask = ~np.isnan(r_coords) & ~np.isnan(phi_coords)
